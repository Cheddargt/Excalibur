#include "stdafx.h"
#include "Chefao.h"
#define KNOCKBACK_X 200
#define KNOCKBACK_Y 120

Chefao::Chefao(sf::Texture* texture, sf::Vector2u imageCount, float switchTime, float speed, int health, int attack, int id) :
...
void Chefao::Update(float deltaTime)
{
	...
	if ((tempoDescida > 1.0f) && !(moveUp))
	{
		moveUp = true;
		tempoDescida = relogio1.restart().asSeconds();
	}
	else if ((tempoDescida > 0.4f) && (moveUp))
	{
		moveUp = false;
		tempoDescida = relogio1.restart().asSeconds();
	}
